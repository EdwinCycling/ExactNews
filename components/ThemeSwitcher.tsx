...
const ThemeSwitcher: React.FC<ThemeSwitcherProps> = ({ theme, toggleTheme, language }) => {
  const t = {
    nl: { label: theme === 'dark' ? 'Activeer lichte modus' : 'Activeer donkere modus', sun: 'Lichte modus', moon: 'Donkere modus' },
    en: { label: theme === 'dark' ? 'Activate light mode' : 'Activate dark mode', sun: 'Light mode', moon: 'Dark mode' },
    de: { label: theme === 'dark' ? 'Lichtmodus aktivieren' : 'Dunkelmodus aktivieren', sun: 'Heller Modus', moon: 'Dunkler Modus' },
  }
  return (
    <button
      onClick={toggleTheme}
