    ...
    <div className="w-full max-w-4xl mx-auto border border-gray-200 dark:border-gray-700 rounded-lg shadow-xl bg-white/50 dark:bg-gray-800/50 backdrop-blur-sm">
      ...
      <div className="h-[500px] overflow-y-auto p-4 space-y-4">
        {history.map((msg, index) => {
          const isSystemMessage = msg.role === 'model' && msg.parts[0].text.startsWith('[SYSTEM]');
          
          if (isSystemMessage) {
            const systemText = msg.parts[0].text.substring('[SYSTEM]'.length).trim();
            return (
              <div key={index} className="flex items-center gap-3 justify-center text-xs text-slate-500 dark:text-slate-400 font-medium my-4">
                <div className="flex-grow border-t border-dashed border-gray-300 dark:border-gray-600"></div>
                <div className="flex-shrink-0 flex items-center gap-2 bg-slate-200 dark:bg-gray-700 rounded-full px-3 py-1.5">
